...
class SystemTask extends Migrator
{
    public function change()
    {
        $this->table('system_task', ['id'=>false,'primary_key'=>'id', 'comment'=>'系统任务表','collation'=>'utf8mb4_general_ci'])
            ...
            ->addColumn(
                Column::string('title')->setLimit(100)->setNull(false)->setComment('任务名称')
            )
            ->addColumn(
                Column::string('command')->setLimit(1000)->setNull(false)->setComment('任务指令')
            )
            ->addColumn(
                Column::string('params')->setLimit(1000)->setNull(true)->setComment('任务参数')
            )
            ->addColumn(
                Column::tinyInteger('type')->setLimit(1)->setSigned(false)->setNull(false)->setComment('任务类型')
            )
            ->addColumn(
                Column::string('crontab')->setLimit(200)->setNull(true)->setComment('定时参数')
            )
            ->addColumn(
